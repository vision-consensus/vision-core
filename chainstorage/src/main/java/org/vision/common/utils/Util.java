...
import org.vision.api.GrpcAPI.TransactionList;
...
public class Util {
  ...
  public static void checkBodySize(String body) throws Exception {
    long max = 4 * 1024 * 1024;
    if (body.getBytes().length > max) {
      throw new Exception("body size is too big, the limit is " + max);
    }
  }
  ...
  public static List<Log> convertLogAddressToVisionAddress(TransactionInfo transactionInfo) {
    ...
    for (Log log : transactionInfo.getLogList()) {
      ...
      if (oldAddress.length == 0 || oldAddress.length > 20) {
        ...
        logBuilder
            .setAddress(ByteString.copyFrom(TransactionTrace.convertToVisionAddress(newAddress)));
      }
