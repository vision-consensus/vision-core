...
import org.vision.common.zksnark.ZksnarkUtils;
...
@Slf4j
public class ShieldedVRC20ParametersBuilder {
  ...
  @Getter
  private ShieldedVRC20Parameters.Builder builder = ShieldedVRC20Parameters.newBuilder();
  @Getter
  private long valueBalance = 0;
  @Getter
  @Setter
  private ShieldedVRC20ParametersType shieldedVRC20ParametersType;
  @Setter
  private byte[] shieldedVRC20Address;
  @Setter
  ...
  public ShieldedVRC20ParametersBuilder() {

  }

  public ShieldedVRC20ParametersBuilder(String type) throws ZksnarkException {
    switch (type) {
      case "mint":
        shieldedVRC20ParametersType = ShieldedVRC20ParametersType.MINT;
        break;
      case "transfer":
        shieldedVRC20ParametersType = ShieldedVRC20ParametersType.TRANSFER;
        break;
      case "burn":
        shieldedVRC20ParametersType = ShieldedVRC20ParametersType.BURN;
        break;
      default:
        throw new ZksnarkException("invalid shielded VRC-20 parameters type");
    }
  }
  ...
  public ShieldedVRC20Parameters build(boolean withAsk) throws ZksnarkException {
    // Empty output script
    byte[] mergedBytes;
    ...
    try {
      switch (shieldedVRC20ParametersType) {
        case MINT:
          ...
          mergedBytes = ByteUtil.merge(shieldedVRC20Address,
        ...
        case TRANSFER:
          ...
          mergedBytes = shieldedVRC20Address;
      ...
      throw new ZksnarkException("build the shielded VRC-20 parameters error: " + e.getMessage());
    } finally {
      JLibrustzcash.librustzcashSaplingProvingCtxFree(ctx);
    }

    if (withAsk || shieldedVRC20ParametersType == ShieldedVRC20ParametersType.MINT) {
      shieldedVRC20Parameters = builder.build();
      builder.setTriggerContractInput(
          getTriggerContractInput(shieldedVRC20Parameters, null, value, true,
              transparentToAddress));
    }

    if (!withAsk && shieldedVRC20ParametersType == ShieldedVRC20ParametersType.BURN) {
      builder.setTriggerContractInput(Hex.toHexString(burnCiphertext));
    }
    return builder.build();
  }

  public String getTriggerContractInput(ShieldedVRC20Parameters shieldedVRC20Parameters,
      List<BytesMessage> spendAuthoritySignature,
      BigInteger value, boolean withAsk,
      byte[] transparentToAddress) {
    switch (shieldedVRC20ParametersType) {
      case MINT:
        return mintParamsToHexString(shieldedVRC20Parameters, value);
      case TRANSFER:
        return transferParamsToHexString(shieldedVRC20Parameters, spendAuthoritySignature, withAsk);
      case BURN:
        return burnParamsToHexString(shieldedVRC20Parameters, spendAuthoritySignature, value,
  ...
  private String mintParamsToHexString(GrpcAPI.ShieldedVRC20Parameters mintParams,
      BigInteger value) {
  ...
  private String transferParamsToHexString(GrpcAPI.ShieldedVRC20Parameters transferParams,
      List<BytesMessage> spendAuthoritySignature,
      boolean withAsk) {
  ...
  private String burnParamsToHexString(GrpcAPI.ShieldedVRC20Parameters burnParams,
      List<BytesMessage> spendAuthoritySignature,
  ...
  public enum ShieldedVRC20ParametersType {
    MINT,
    TRANSFER,
    BURN
  }
