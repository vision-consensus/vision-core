package org.vision.core.metrics.net;
...
public class NetMetricManager {
  ...
  private void setNetInfo(NetInfo netInfo) {
    //set connection info
    netInfo.setConnectionCount(visionNetDelegate.getActivePeer().size());
    int validConnectionCount = 0;
    for (PeerConnection peerConnection : visionNetDelegate.getActivePeer()) {
