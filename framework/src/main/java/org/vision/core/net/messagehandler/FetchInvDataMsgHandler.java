...
import org.vision.common.overlay.discover.node.statistics.MessageCount;
...
import org.vision.core.net.message.PbftCommitMessage;
...
@Slf4j(topic = "net")
@Component
public class FetchInvDataMsgHandler implements VisionMsgHandler {
  ...
  public void processMessage(PeerConnection peer, VisionMessage msg) throws P2pException {
    ...
    for (Sha256Hash hash : fetchInvDataMsg.getHashList()) {
      ...
      if (message == null) {
        try {
          message = visionNetDelegate.getData(hash, type);
        } catch (Exception e) {
  ...
  private void sendPbftCommitMessage(PeerConnection peer, BlockCapsule blockCapsule) {
    try {
      if (!visionNetDelegate.allowPBFT() || peer.isSyncFinish()) {
        return;
      }
      long epoch = 0;
      PbftSignCapsule pbftSignCapsule = visionNetDelegate
      ...
      if (epochCache.getIfPresent(epoch) == null) {
        PbftSignCapsule srl = visionNetDelegate.getSRLPbftCommitData(epoch);
        if (srl != null) {
  ...
  private void check(PeerConnection peer, FetchInvDataMessage fetchInvDataMsg) throws P2pException {
    ...
    if (type == MessageTypes.TRX) {
      ...
      int fetchCount = peer.getNodeStatistics().messageStatistics.visionInTrxFetchInvDataElement
          .getCount(10);
      ...
      if (isAdv) {
        MessageCount visionOutAdvBlock = peer.getNodeStatistics().messageStatistics.visionOutAdvBlock;
        visionOutAdvBlock.add(fetchInvDataMsg.getHashList().size());
        int outBlockCountIn1min = visionOutAdvBlock.getCount(60);
