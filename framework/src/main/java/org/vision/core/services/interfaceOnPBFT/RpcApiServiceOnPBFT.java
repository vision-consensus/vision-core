...
import org.vision.api.GrpcAPI.BlockExtention;
import org.vision.api.GrpcAPI.BlockReference;
...
import org.vision.api.GrpcAPI.NullifierResult;
...
import org.vision.protos.Protocol.DynamicProperties;
...
public class RpcApiServiceOnPBFT implements Service {
  ...
  private class WalletPBFTApi extends WalletSolidityImplBase {
    ...
    @Override
    public void scanShieldedVRC20NotesByIvk(GrpcAPI.IvkDecryptVRC20Parameters request,
                                            StreamObserver<GrpcAPI.DecryptNotesVRC20> responseObserver) {
      walletOnPBFT.futureGet(
          () -> rpcApiService.getWalletSolidityApi()
          .scanShieldedVRC20NotesByIvk(request, responseObserver)
      );
    }

    @Override
    public void scanShieldedVRC20NotesByOvk(GrpcAPI.OvkDecryptVRC20Parameters request,
                                            StreamObserver<GrpcAPI.DecryptNotesVRC20> responseObserver) {
      walletOnPBFT.futureGet(
          () -> rpcApiService.getWalletSolidityApi()
          .scanShieldedVRC20NotesByOvk(request, responseObserver)
      );
    }

    @Override
    public void isShieldedVRC20ContractNoteSpent(GrpcAPI.NfVRC20Parameters request,
                                                 StreamObserver<NullifierResult> responseObserver) {
      walletOnPBFT.futureGet(
          () -> rpcApiService.getWalletSolidityApi()
          .isShieldedVRC20ContractNoteSpent(request, responseObserver)
      );
    }
    ...
    public void getBurnVs(EmptyMessage request, StreamObserver<NumberMessage> responseObserver) {
      walletOnPBFT.futureGet(
          () -> rpcApiService.getWalletSolidityApi().getBurnVs(request, responseObserver)
      );
    }
  }
}
