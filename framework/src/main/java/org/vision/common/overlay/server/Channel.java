...
import org.vision.common.overlay.message.MessageCodec;
import org.vision.common.overlay.message.StaticMessages;
import org.vision.core.db.ByteArrayWrapper;
import org.vision.core.exception.P2pException;
import org.vision.core.net.PbftHandler;
import org.vision.core.net.VisionNetHandler;
...
public class Channel {
  ...
  @Autowired
  private VisionNetHandler visionNetHandler;
  @Autowired
  ...
  private VisionState visionState = VisionState.INIT;
  ...
  public void init(ChannelPipeline pipeline, String remoteId, boolean discoveryMode,
    ...
    pipeline.addLast("lengthDecode", new VSProtobufVarint32FrameDecoder(this));
    ...
    visionNetHandler.setChannel(this);
    pbftHandler.setChannel(this);

    p2pHandler.setMsgQueue(msgQueue);
    visionNetHandler.setMsgQueue(msgQueue);
  ...
  public void publicHandshakeFinished(ChannelHandlerContext ctx, HelloMessage msg) {
    ...
    ctx.pipeline().addLast("data", visionNetHandler);
    ctx.pipeline().addLast("pbft", pbftHandler);
    setStartTime(msg.getTimestamp());
    setVisionState(VisionState.HANDSHAKE_FINISHED);
  ...
  public void setVisionState(VisionState visionState) {
    ...
    logger.info("Peer {} status change to {}.", inetSocketAddress, visionState);
  }
