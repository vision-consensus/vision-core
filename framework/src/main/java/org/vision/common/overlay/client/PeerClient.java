...
import org.vision.common.overlay.server.VisionChannelInitializer;
...
public class PeerClient {
  ...
  public PeerClient() {
    workerGroup = new NioEventLoopGroup(0, new ThreadFactory() {
      ...
      public Thread newThread(Runnable r) {
        return new Thread(r, "VisionJClientWorker-" + cnt.getAndIncrement());
      }
    });
  }
  ...
  private ChannelFuture connectAsync(String host, int port, String remoteId,
    ...
    VisionChannelInitializer visionChannelInitializer = ctx
        .getBean(VisionChannelInitializer.class, remoteId);
    visionChannelInitializer.setPeerDiscoveryMode(discoveryMode);
    ...
    b.handler(visionChannelInitializer);
