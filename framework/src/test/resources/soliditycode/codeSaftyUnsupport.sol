...
function() payable external{}
mapping(address => mapping(vrcToken => uint256)) sender_tokens;

function trigger1(address payable addr, vrcToken tokenInputId) payable public {
...
function trigger6(address payable addr, vrcToken tokenId) payable public {
address(SubC(addr)).call.value(1000)(abi.encodeWithSignature("transferToken(uint256, vrcToken)", 10, tokenId));
}
