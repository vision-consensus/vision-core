...
contract ShieldedVRC20 {
    using SafeMath for uint256;

    uint256 public scalingFactor; // used when decimals of VRC20 token is too large.
    ...
    constructor (address vrc20ContractAddress, uint256 scalingFactorExponent) public {
        require(scalingFactorExponent < 77, "The scalingFactorExponent is out of range!");
        scalingFactor = 10 ** scalingFactorExponent;
        owner = msg.sender;
        vrc20Token = TokenVRC20(vrc20ContractAddress);
    }
    ...
    function mint(uint256 rawValue, bytes32[9] calldata output, bytes32[2] calldata bindingSignature, bytes32[21] calldata c) external {
        ...
        // transfer the vrc20Token from the sender to this contract
        bool transferResult = vrc20Token.transferFrom(sender, address(this), rawValue);
    ...
    function burn(bytes32[10] calldata input, bytes32[2] calldata spendAuthoritySignature, uint256 rawValue, bytes32[2] calldata bindingSignature, address payTo, bytes32[3] calldata burnCipher, bytes32[9][] calldata output, bytes32[21][] calldata c) external {
        ...
        nullifiers[nf] = nf;
        emit NoteSpent(nf);
        //Finally, transfer vrc20Token from this contract to the nominated address
        bool transferResult = vrc20Token.transfer(payTo, rawValue);
