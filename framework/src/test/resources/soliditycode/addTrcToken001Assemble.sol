...
mapping(vrcToken => uint256) tokenCnt;
mapping(uint256 => mapping(vrcToken => vrcToken)) cntTokenToken;
...
function getTokenBalanceConstant  (address addr, vrcToken tokenId) view public returns(uint256 r) {
assembly{
r := tokenbalance(tokenId, addr)
}
}

function getTokenBalance (address addr, vrcToken tokenId) public returns(uint256 r) {
assembly{
r := tokenbalance(tokenId, addr)
}
}

function transferTokenInAssembly(address addr, vrcToken tokenId, uint256 tokenValue) public payable {
...
function vrcTokenInMap(vrcToken tokenId, uint256 tokenValue) public returns(uint256 r) {
tokenCnt[tokenId] += tokenValue;
r = tokenCnt[tokenId];
}

function cntTokenTokenInMap(vrcToken tokenId1, vrcToken tokenId2, uint256 tokenValue) public returns(vrcToken r) {
