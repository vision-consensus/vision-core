...
import static org.vision.protos.Protocol.TransactionInfo.code.SUCESS;
...
@Slf4j
public class ContractVrcToken018 {
  ...
  public void testDeployTransferTokenContract() {
    ...
    String filePath = "./src/test/resources/soliditycode/contractVrcToken023.sol";
    ...
    logger.info("Delpoy entropytotal is " + infoById.get().getReceipt().getEntropyUsageTotal());
    ...
    final String triggerTxid = PublicMethed
        .triggerContract(transferTokenContractAddress, "TransferTokenTo(address,vrcToken,uint256)",
    ...
    logger.info("infoById.get().getResMessage().toStringUtf8(): " + infoById.get().getResMessage()
        .toStringUtf8());
    /*Assert.assertEquals(
        "transfer vrc10 failed: Validate InternalTransfer error, no ToAccount. "
