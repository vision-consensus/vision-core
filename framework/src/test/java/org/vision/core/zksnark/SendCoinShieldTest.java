package org.vision.core.zksnark;
...
import org.vision.common.zksnark.*;
...
import org.vision.common.zksnark.MerklePath;
import org.vision.common.zksnark.ZksnarkClient;
...
import org.vision.core.capsule.ReceiveDescriptionCapsule;
...
public class SendCoinShieldTest {
  ...
  private static final String ASSET_NAME = "vs";
  ...
  public void createCapsule() {
    ...
    AssetIssueContract assetIssueContract = AssetIssueContract.newBuilder()
        ...
        .setId(Long.toString(tokenId)).setTotalSupply(OWNER_BALANCE).setVsNum(VS_NUM).setNum(NUM)
  ...
  public void testGenerateSpendProof() throws Exception {
    ...
    ZenTransactionBuilder.SpendDescriptionInfo spend = new ZenTransactionBuilder.SpendDescriptionInfo(expsk, note, anchor, voucher);
  ...
  public void verifyOutputProof() throws ZksnarkException {
    ...
    if (!JLibrustzcash.librustzcashSaplingCheckOutput(
        new LibrustzcashParam.CheckOutputParams(ctx, receiveDescription.getValueCommitment().toByteArray(),
  ...
  public void testDecryptReceiveWithIvk() throws ZksnarkException {
    ...
    long ctx = JLibrustzcash.librustzcashSaplingProvingCtxInit();
    byte[] memo = org.vision.keystore.Wallet.generateRandomBytes(512);
    ...
    if (!JLibrustzcash.librustzcashIvkToPkd(
        new LibrustzcashParam.IvkToPkdParams(incomingViewingKey.getValue(), noteText.getD().getData(), pkD))) {
      JLibrustzcash.librustzcashSaplingProvingCtxFree(ctx);
      return;
    }
  ...
  public void testDecryptReceiveWithOvk() throws Exception {
    ...
    Note.NotePlaintextEncryptionResult enc = note.encrypt(pkd).get();
    ...
    // encrypt with ovk
    NoteEncryption.Encryption.OutCiphertext outCiphertext = outgoingPlaintext
  ...
  public void pushShieldedTransactionAndDecryptWithIvk()
    ...
    for (org.vision.protos.Protocol.Transaction.Contract c : t.getRawData().getContractList()) {
  ...
  public void testVerifySpendProof() throws BadItemException, ZksnarkException {
    ...
    JLibrustzcash.librustzcashSaplingSpendSig(
        new LibrustzcashParam.SpendSigParams(expsk.getAsk(), spend.getAlpha(), getHash(), result));

    long verifyContext = JLibrustzcash.librustzcashSaplingVerificationCtxInit();
    boolean ok = JLibrustzcash.librustzcashSaplingCheckSpend(new LibrustzcashParam.CheckSpendParams(verifyContext,
  ...
  public void saplingBindingSig() throws BadItemException, ZksnarkException {
    ...
    boolean ret = JLibrustzcash.librustzcashSaplingBindingSig(
        new LibrustzcashParam.BindingSigParams(ctx, builder.getValueBalance(), getHash(), bindingSig));
    JLibrustzcash.librustzcashSaplingProvingCtxFree(ctx);
    Assert.assertTrue(ret);
  }
  ...
  public void finalCheck() throws BadItemException, ZksnarkException {
    ...
    byte[] result = new byte[64];
    JLibrustzcash.librustzcashSaplingSpendSig(
        new LibrustzcashParam.SpendSigParams(expsk.getAsk(), builder.getSpends().get(0).getAlpha(), getHash(),
            result));

    SpendDescription spendDescription = spendDescriptionCapsule.getInstance();
    boolean ok;
    ok = JLibrustzcash.librustzcashSaplingCheckSpend(
        new LibrustzcashParam.CheckSpendParams(ctx, spendDescription.getValueCommitment().toByteArray(),
    ...
    ok = JLibrustzcash.librustzcashSaplingFinalCheck(
        new LibrustzcashParam.FinalCheckParams(ctx, builder.getValueBalance(), bindingSig, getHash()));
    Assert.assertTrue(ok);
    JLibrustzcash.librustzcashSaplingVerificationCtxFree(ctx);
  }
  ...
  public void testEmptyRoots() throws Exception {
    ...
    for (int i = 0; i < 32; i++) {
      ...
      IncrementalMerkleTreeContainer.EmptyMerkleRoots emptyMerkleRootsInstance = IncrementalMerkleTreeContainer.EmptyMerkleRoots.getEmptyMerkleRootsInstance();
  ...
  public void testComputeCm() throws Exception {
    ...
    if (!JLibrustzcash.librustzcashComputeCm(
        new LibrustzcashParam.ComputeCmParams((ByteArray.fromHexString("fc6eb90855700861de6639")), ByteArray
  ...
  public void TestGeneratesProofWithWrongAlpha() throws Exception {
    ...
    //case 3， no public input, an input cm,  no output cm, a public output
    {
      ...
      ZenTransactionBuilder.SpendDescriptionInfo spendDescriptionInfo = new ZenTransactionBuilder.SpendDescriptionInfo(expsk2, note2, anchor2,
          voucher2);
      byte[] bytes = ByteArray
      ...
      byte[] result = new byte[64];
      JLibrustzcash.librustzcashSaplingSpendSig(
          new LibrustzcashParam.SpendSigParams(spendDescriptionInfo.getExpsk().getAsk(),
      ...
      ZenTransactionBuilder builder = new ZenTransactionBuilder(wallet) {
        ...
        public void createSpendAuth(byte[] dataToBeSigned) throws ZksnarkException {
          for (int i = 0; i < this.getSpends().size(); i++) {
            byte[] result = new byte[64];
            JLibrustzcash.librustzcashSaplingSpendSig(
                new LibrustzcashParam.SpendSigParams(fakeAsk, this.getSpends().get(i).getAlpha(), dataToBeSigned,
                    result));
  ...
  private ZenTransactionBuilder.SpendDescriptionInfo generateDefaultSpend() throws BadItemException, ZksnarkException {
    SpendingKey sk = SpendingKey.random();
    ExpandedSpendingKey expsk = sk.expandedSpendingKey();
    PaymentAddress address = sk.defaultAddress();
    Note note = new Note(address, 1000 * 1000000L);
    ...
    return new ZenTransactionBuilder.SpendDescriptionInfo(expsk, note, anchor, voucher);
  }
  ...
  private TransactionCapsule generateDefaultBuilder(ZenTransactionBuilder builder)
    ...
    //add spendDesc into builder
    ZenTransactionBuilder.SpendDescriptionInfo spendDescriptionInfo = generateDefaultSpend();
