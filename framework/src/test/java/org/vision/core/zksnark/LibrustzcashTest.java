...
import org.vision.common.zksnark.JLibsodium;
...
public class LibrustzcashTest {
  ...
  public static void test(byte[] K, byte[] ovk, byte[] cv, byte[] cm, byte[] epk)
    ...
    Assert.assertTrue(
        JLibsodium.cryptoGenerichashBlack2bSaltPersonal(
            new JLibsodiumParam.Black2bSaltPersonalParams(K, 32, block, 128, null, 0, // No key.
                null,    // No salt.
                personalization)) == 0);

    byte[] cipher_nonce = new byte[JLibsodium.CRYPTO_AEAD_CHACHA20POLY1305_IETF_NPUBBYTES];
    Assert.assertTrue(JLibsodium
        .cryptoAeadChacha20poly1305IetfDecrypt(new JLibsodiumParam.Chacha20poly1305IetfDecryptParams(
  ...
  public void testZcashParam() throws ZksnarkException {
    ...
    byte[] r = {(byte) 0xb7, 0x2c, (byte) 0xf7, (byte) 0xd6, 0x5e, 0x0e, (byte) 0x97, (byte) 0xd0,
        ...
        (byte) 0xb4, 0x7d, 0x0e};
    byte[] cm = new byte[32];
    boolean check_d = JLibrustzcash.librustzcashCheckDiversifier(d);
    Assert.assertTrue(check_d);

    //Most significant five bits of ivk must be 0.
    ivk[31] = (byte) 0x07;
    boolean check_pkd = JLibrustzcash.librustzcashIvkToPkd(new LibrustzcashParam.IvkToPkdParams(ivk, d, pkD));
    ...
    boolean res = JLibrustzcash.librustzcashComputeCm(new LibrustzcashParam.ComputeCmParams(d, pkD, value, r, cm));
    ...
    boolean boolSigRes = JLibrustzcash.librustzcashSaplingSpendSig(
        new LibrustzcashParam.SpendSigParams(ask, alpha, sighash, sigRes));
    ...
    byte[] nk = new byte[32];
    nk = JLibrustzcash.librustzcashNskToNk(nsk);
    ...
    boolean boolBindSig = JLibrustzcash.librustzcashSaplingBindingSig(
        new LibrustzcashParam.BindingSigParams(ctx, value, sighash, resbindSig));
  ...
  public long benchmarkCreateSpend() throws ZksnarkException {
    ...
    boolean ret;
    ret = JLibrustzcash.librustzcashSaplingSpendProof(new LibrustzcashParam.SpendProofParams(ctx, ak,
        nsk,
        d,
        rcm,
        alpha,
  ...
  public long benchmarkCreateSaplingOutput() throws BadItemException, ZksnarkException {
    ...
    Optional<Note.NotePlaintextEncryptionResult> res = note.encrypt(note.getPkD());
    if (!res.isPresent()) {
  ...
  public void testPedersenHash() throws Exception {
    ...
    JLibrustzcash.librustzcashMerkleHash(new LibrustzcashParam.MerkleHashParams(25, a, b, res));
