...
public class ShieldedReceiveTest extends BlockGenerate {
  ...
  private static byte[] randomUint256() {
    return org.vision.keystore.Wallet.generateRandomBytes(32);
  }

  private static byte[] randomUint640() {
    return org.vision.keystore.Wallet.generateRandomBytes(80);
  }

  private static byte[] randomUint1536() {
    return org.vision.keystore.Wallet.generateRandomBytes(192);
  }

  private static byte[] randomUint4640() {
    return org.vision.keystore.Wallet.generateRandomBytes(580);
  }
  ...
  public String[] generateSpendAndOutputParams() throws ZksnarkException, BadItemException {
    ...
    for (ZenTransactionBuilder.SpendDescriptionInfo spend2 : builder.getSpends()) {
    ...
    for (ZenTransactionBuilder.ReceiveDescriptionInfo receive : builder.getReceives()) {
    ...
    LibrustzcashParam.CheckSpendParams checkSpendParams = new LibrustzcashParam.CheckSpendParams(ctx,
    ...
    LibrustzcashParam.CheckOutputParams checkOutputParams = new LibrustzcashParam.CheckOutputParams(ctx,
  ...
  private long benchmarkVerifySpend(String spend, String dataToBeSigned) throws ZksnarkException {
    ...
    LibrustzcashParam.CheckSpendParams checkSpendParams = LibrustzcashParam.CheckSpendParams.decode(ctx,
  ...
  private long benchmarkVerifyOutput(String outputParams) throws ZksnarkException {
    ...
    LibrustzcashParam.CheckOutputParams checkOutputParams = LibrustzcashParam.CheckOutputParams.decode(ctx,
  ...
  private ZenTransactionBuilder generateBuilderWithoutColumnInDescription(
    ...
    // Create Sapling SpendDescriptions
    for (ZenTransactionBuilder.SpendDescriptionInfo spend : builder.getSpends()) {
   ...
   */
  private ReceiveDescriptionCapsule changeGenerateOutputProof(ZenTransactionBuilder.ReceiveDescriptionInfo output,
 ...
 */
  ...
  public void testDiversifierT() throws ZksnarkException {
    //byte[] data = Wallet.generateRandomBytes(Constant.ZC_DIVERSIFIER_SIZE);
  ...
  public TransactionCapsule generateTransactionCapsule(ZenTransactionBuilder builder,
    ...
    for (int i = 0; i < builder.getSpends().size(); i++) {
      ...
      JLibrustzcash.librustzcashSaplingSpendSig(
          new LibrustzcashParam.SpendSigParams(builder.getSpends().get(i).getExpsk().getAsk(),
  ...
  public void testIsolateSignature()
    ...
    List<ZenTransactionBuilder.SpendDescriptionInfo> spends = builder.getSpends();
  ...
  public TransactionCapsule buildShieldedTransactionWithoutSpendAuthSig(PaymentAddress address,
    ...
    chainBaseManager.getMerkleContainer()
        .putMerkleTreeIntoStore(anchor, voucher.getVoucherCapsule().getTree());
    ZenTransactionBuilder.SpendDescriptionInfo skSpend = new ZenTransactionBuilder.SpendDescriptionInfo(ak,
        nsk,
        ovk,
        note,
        Note.generateR(),
  ...
  public void testMemoTooLong() throws ContractValidateException, TooBigTransactionException,
    ...
    byte[] memo = org.vision.keystore.Wallet.generateRandomBytes(1024);
  ...
  public void testMemoNotEnough() throws ContractValidateException, TooBigTransactionException,
    ...
    IncomingViewingKey incomingViewingKey = fullViewingKey.inViewingKey();
    PaymentAddress paymentAddress = incomingViewingKey.address(new DiversifierT()).get();
    byte[] memo = org.vision.keystore.Wallet.generateRandomBytes(128);
    builder.addOutput(expsk.getOvk(), paymentAddress,
        100 * 1000000L - wallet.getShieldedTransactionFee(), memo);
  ...
  public void pushSameSkAndScanAndSpend() throws Exception {
    ...
    for (int i = 0; i < 1; i++) {
      org.vision.api.GrpcAPI.Note grpcNote = notes1.getNoteTxs(i).getNote();
    ...
    byte[] memo3 = org.vision.keystore.Wallet.generateRandomBytes(512);
