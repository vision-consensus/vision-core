...
import static org.vision.common.utils.Commons.adjustAssetBalanceV2;
...
public class ManagerTest extends BlockGenerate {
  ...
  public void GetterInstanceTest() {
    ...
    Assert.assertTrue(Commons.getExchangeStoreFinal(chainManager.getDynamicPropertiesStore(),
  ...
  public void adjustBalanceTest() {
    ...
    try {
      Commons.adjustBalance(chainManager.getAccountStore(), accountAddress.getBytes(), 0);
    ...
    try {
      Commons.adjustBalance(chainManager.getAccountStore(), accountAddress.getBytes(), -40);
      Assert.assertTrue(false);
    } catch (BalanceInsufficientException e) {
      Assert.assertEquals(
    ...
    chainManager.getAccountStore().put(account.createDbKey(), account); // update balance
    try {
      Commons.adjustBalance(chainManager.getAccountStore(), accountAddress.getBytes(), -10);
      AccountCapsule copyAccount = chainManager.getAccountStore().get(ownerAddress);
  ...
  public void adjustAssetBalanceV2Test() {
    ...
    try {
      Commons.adjustAssetBalanceV2(accountAddress.getBytes(), assetID, -20,
    ...
    try {
      Commons.adjustAssetBalanceV2(accountAddress.getBytes(), assetID, 10,
          chainManager.getAccountStore(), chainManager.getAssetIssueStore(),
          chainManager.getDynamicPropertiesStore());
      AccountCapsule copyAccount = chainManager.getAccountStore().get(ownerAddress);
  ...
  public void switchBack()
    ...
    try {
      ...
      dbManager.pushBlock(blockCapsule2);
    } catch (ValidateScheduleException | P2pVersionException e) {
