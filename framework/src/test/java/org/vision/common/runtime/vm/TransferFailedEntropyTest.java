...
public class TransferFailedEntropyTest extends VMTestBase {
  ...
  contract EntropyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferVsInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendVsInsufficientBalance() payable public{
          msg.sender.send(10);
      }

      function testTransferTokenInsufficientBalance(vrcToken tokenId) payable public{
          msg.sender.transferToken(10, tokenId);
      }

      function testCallVsInsufficientBalance(address payable caller) public {
      ...
      // NonexistentTarget
      function testTransferVsNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testTransferTokenNonexistentTarget(address payable nonexistentTarget,
       vrcToken tokenId) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transferToken(10, tokenId);
      }

      function testCallVsNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferVsSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendVsSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }

      function testTransferTokenSelf(vrcToken tokenId) payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
  ...
  contract EntropyOfTransferFailedTest {
      ...
      function testCallVsInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferVsNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallVsNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  @Test
  public void testTransferFailedAfterAllowVvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
          VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowVvmTransferVrc10(1);
    ...
    // deploy contract
    Transaction trx = VvmTestUtils.generateDeploySmartContractAndGetTransaction(
    ...
    runtime = VvmTestUtils.processTransactionAndReturnRuntime(trx, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());

    for (TestCase testCase : testCasesAfterAllowVvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }

    trx = VvmTestUtils.generateDeploySmartContractAndGetTransaction(
  ...
  @Test
  public void testTransferFailedBeforeAllowVvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowVvmTransferVrc10(1);
    ...
    for (TestCase testCase : testCasesBeforeAllowVvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }
  ...
  private void checkResult(TestCase testCase, byte[] factoryAddress)
    ...
    if (testCase.allEntropy) {
      Assert.assertEquals(programResult.getEntropyUsed(), 20000000, testCase.getMethod());
    } else {
