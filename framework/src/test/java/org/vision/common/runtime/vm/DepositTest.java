...
public class DepositTest {
  ...
  public void loopCallTest()
    ...
    Transaction aTrx = VvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null, engeryLiimt);
    Runtime runtime = VvmTestUtils
        .processTransactionAndReturnRuntime(aTrx, DepositImpl.createRoot(manager),
            null);
    Assert.assertNull(runtime.getRuntimeError());

    Transaction bTrx = VvmTestUtils.generateDeploySmartContractAndGetTransaction(
    ...
    // check result
    // expected: n1 = 1; n2 = 0
    byte[] checkN1Data = VvmTestUtils.parseAbi("n1()", null);
    byte[] checkN2Data = VvmTestUtils.parseAbi("n2()", null);

    VVMTestResult checkN1 = VvmTestUtils
        .triggerContractAndReturnVvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    VVMTestResult checkN2 = VvmTestUtils
        .triggerContractAndReturnVvmTestResult(Hex.decode(OWNER_ADDRESS),
    ...
    triggerData = VvmTestUtils.parseAbi("callBcallA(address,uint256,uint256)", params2);
    result = VvmTestUtils
        .triggerContractAndReturnVvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, triggerData, 0, fee, manager, null);
    Assert.assertNull(result.getRuntime().getRuntimeError());
    checkN1 = VvmTestUtils
        .triggerContractAndReturnVvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    checkN2 = VvmTestUtils
        .triggerContractAndReturnVvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN2Data, 0, fee, manager, null);
    System.out.println(Hex.toHexString(checkN1.getRuntime().getResult().getHReturn()));
    System.out.println(Hex.toHexString(checkN2.getRuntime().getResult().getHReturn()));
  ...
  public void loopCallTestOldVersion()
    ...
    Runtime runtime = VvmTestUtils.processTransactionAndReturnRuntime(aTrx, rootDeposit,
        null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    Transaction bTrx = VvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    rootDeposit = DepositImpl.createRoot(manager);
    runtime = VvmTestUtils.processTransactionAndReturnRuntime(bTrx, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
