...
public class Create2Test extends VMTestBase {
  ...
  */
  ...
  public void testCreate2()
    ...
    manager.getDynamicPropertiesStore().saveAllowVvmIstanbul(0);
    ...
    byte[] actualContract = TransactionTrace.convertToVisionAddress(Arrays.copyOfRange(returnValue,
        12, 32));
    ...
    byte[] expectedContract =
        WalletUtil.generateContractAddress2(address, new DataWord(salt).getData(), Hex.decode(testCode));
...
contract A {
  ...
  public void create2AddressTest() throws ContractExeException, ReceiptCheckErrException,
    ...
    manager.getDynamicPropertiesStore().saveAllowVvmTransferVrc10(1);
    manager.getDynamicPropertiesStore().saveAllowVvmConstantinople(1);
    manager.getDynamicPropertiesStore().saveAllowVvmSolidity059(1);
    manager.getDynamicPropertiesStore().saveAllowVvmIstanbul(1);
    String contractName = "Factory_0";
    byte[] address = Hex.decode(OWNER_ADDRESS);
    String abi = "[]";
    ...
    byte[] returnValue = result.getRuntime().getResult().getHReturn();
    byte[] actualContract = convertToVisionAddress(Arrays.copyOfRange(returnValue,
        12, 32));
    byte[] expectedContract =
    ...
    // same input
    result = VvmTestUtils.triggerContractAndReturnVvmTestResult(Hex.decode(OWNER_ADDRESS),
              factoryAddress, Hex.decode(hexInput), 0, fee, manager, null);
    ...
    result = VvmTestUtils
        .triggerContractAndReturnVvmTestResult(Hex.decode(ownerAddress2),
            factoryAddress, Hex.decode(hexInput), 0, fee, manager, null);
