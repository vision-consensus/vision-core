...
import org.vision.common.crypto.jce.ECKeyFactory;
import org.vision.common.crypto.jce.ECKeyPairGenerator;
import org.vision.common.crypto.jce.VisionCastleProvider;
...
public class ECKey implements Serializable, SignInterface {
  ...
  public ECKey(SecureRandom secureRandom) {
    this(VisionCastleProvider.getInstance(), secureRandom);
  }
  ...
  public ECKey(byte[] key, boolean isPrivateKey) {
    ...
    this.provider = VisionCastleProvider.getInstance();
  }
  ...
  public ECKey(@Nullable BigInteger priv, ECPoint pub) {
    this(
        VisionCastleProvider.getInstance(),
   ...
   * as the VdtPKCS11 Provider will return a generic PrivateKey instance
  ...
  private static PrivateKey privateKeyFromBigInteger(BigInteger priv) {
    if (priv == null) {
      ...
      try {
        return ECKeyFactory
            .getInstance(VisionCastleProvider.getInstance())
